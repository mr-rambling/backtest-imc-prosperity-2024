import sys
import os
import dontlooseshells_algo as Trader
from jmerle_logger import Logger
...
TRAINING_DATA_PREFIX = "backtest-imc-prosperity-2024/training/2024/"
...
tutorial_pst = ['AMETHYSTS', 'STARFRUIT']
...
def process_prices(df_prices, round, time_limit) -> dict[int, TradingState]:
    ...
    for _, row in df_prices.iterrows():
        ...
        if states.get(time) == None:
            ...
            listings = {}
            depths = {}
            states[time] = TradingState(traderData, time, listings, depths, own_trades, market_trades, observations, position)
        ...
        if row["bid_price_3"]> 0:
            depth.buy_orders[int(row["bid_price_3"])] = int(row["bid_volume_3"])
        ...
        if row["ask_price_3"]> 0:
            depth.sell_orders[int(row["ask_price_3"])] = -int(row["ask_volume_3"])
...
def simulate_alternative(
    ...
    # handling these four is rather tricky 
        # deep copy creates a complete duplication of the whole memory structure of the copied variable
    # The final pnl
    profits_by_symbol: dict[Time, dict[Symbol, float]] = { 0: dict(zip(ref_symbols, [0.0]*len(ref_symbols))) }
    # Credit + unrealised
    balance_by_symbol: dict[Time, dict[Symbol, float]] = { 0: copy.deepcopy(profits_by_symbol[0]) }
    # Credit taken to open a position
    credit_by_symbol: dict[Time, dict[Symbol, float]] = { 0: copy.deepcopy(profits_by_symbol[0]) }
    # Value (mid_price) if position was closed
    unrealized_by_symbol: dict[Time, dict[Symbol, float]] = { 0: copy.deepcopy(profits_by_symbol[0]) }

    logger: dict[Time, str] = {}

    states, trader, logger, profits_by_symbol, balance_by_symbol = trades_position_pnl_run(states, max_time, profits_by_symbol, balance_by_symbol, credit_by_symbol, unrealized_by_symbol)
    create_csv(states, profits_by_symbol)
    create_log_file(round, day, states, profits_by_symbol, balance_by_symbol, logger)
...
def trades_position_pnl_run(
    states: dict[Time, TradingState],
    max_time: int, 
    profits_by_symbol: dict[Time, dict[Symbol, float]], 
    balance_by_symbol: dict[Time, dict[Symbol, float]], 
    credit_by_symbol: dict[Time, dict[Symbol, float]], 
    unrealized_by_symbol: dict[Time, dict[Symbol, float]], 
    ):
    ...
    return states, trader, logger, profits_by_symbol, balance_by_symbol
...
def clear_order_book(player_orders: dict[str, List[Order]], order_depth: dict[str, OrderDepth], time: int, halfway: bool) -> list[Trade]:
        trades = []
        for symbol in player_orders.keys():
            if order_depth.get(symbol) != None:
                symbol_order_depth = copy.deepcopy(order_depth[symbol])
                t_orders = cleanup_order_volumes(player_orders[symbol])
                for order in player_orders[symbol]:
                    if order.quantity < 0:
                        if halfway:
...
def create_log_file(round: int, day: str, states: dict[int, TradingState], profits_by_symbol: dict[int, dict[str, float]], balance_by_symbol: dict[int, dict[str, float]], logger: dict[Time, str]):
    file_name = uuid.uuid4()
    timest = datetime.timestamp(datetime.now())
    max_time = max(list(states.keys()))
    log_path = os.path.join('backtest-imc-prosperity-2024/logs', f'{timest}_{file_name}.txt')
    with open(log_path, 'w', encoding="utf-8", newline='\n') as f:
        f.writelines(log_header)
        for time, state in states.items():
            f.write('{\n')
            f.write('  \"sandboxLog\": \"\",\n')
            f.writelines(['  \"lambdaLog\": ', json.dumps(logger[time]), ',\n'])
...
def create_csv(states: dict[Time, TradingState], profits_by_symbol: dict[Time, dict[Symbol, float]]):

    player_df = pd.DataFrame(states.keys(), columns=['timestamp'])
    totalpnl = []
    for ticker in TICKERS:
        profits = []
        positions = []
        for time in states.keys():
            positions.append(states[time].position[TICKERS[ticker]])
            profits.append(profits_by_symbol[time][TICKERS[ticker]])  
        player_df[ticker + '_pnl'] = profits
        player_df[ticker + '_pos'] = positions
    for time in states.keys():
        totalpnl.append(sum(profits_by_symbol[time].values()))
    player_df['total_pnl'] = totalpnl
    player_df.to_csv('backtest-imc-prosperity-2024/simulator/simulator_player.csv', index=False, header=True)
...
if __name__ == "__main__":
    ...
    if max_time < 10:
        max_time *= 100000
    round = int(input("Input a round (blank for 0): ") or 0)
    day = str(input("Input a day (blank for 1, all for concatted): ") or 1)
